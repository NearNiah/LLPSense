...
def iupred(seq, mode='long', new_smoothing=False):
    if mode == "short":
        ...
        uc = 25
        wc = 10
        mtx = read_matrix("{}/iupred/iupred2_short_energy_matrix".format(PATH))
        histo, histo_min, histo_max, histo_step = read_histo("{}/iupred/short_histogram".format(PATH))

    elif mode == 'glob':
        lc = 1
        uc = 100
        wc = 15
        mtx = read_matrix("{}/iupred/iupred2_long_energy_matrix".format(PATH))
        histo, histo_min, histo_max, histo_step = read_histo("{}/iupred/long_histogram".format(PATH))
...
def anchor2(seq):
    ...
    mtx = read_matrix('{}/iupred/anchor2_energy_matrix'.format(PATH))
    interface_comp = {}
    with open('{}/iupred/anchor2_interface_comp'.format(PATH)) as _fn:
        for line in _fn:
